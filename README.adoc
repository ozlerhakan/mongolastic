...
The query above will copy the tweets where the screen name is 'FCBarcelona' from a running elastic node to a mongod listening on localhost:27017. You should see an type called _tweets_ containing all the corresponding tweets tweeted by FCBarcelona in an index called _barca_.
